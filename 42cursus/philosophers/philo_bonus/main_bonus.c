...
/*   Updated: 2023/01/02 11:26:52 by wonyang          ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
static void	*monitor_dead(void *arg)
{
	...
	while (1)
	{
		...
		{
			ft_sem_wait(info->print_sem);
			diff = (get_time() - info->start_time) / 1000;
			printf("%llu %d %s\n", diff, philo->id, "died");
			ft_sem_post(info->end_sem);
			return (NULL);
		}
		msleep(5);
	}
...
static void	child_process(t_philo *philo)
{
	...
	if (!sem_name)
		ft_sem_post(philo->info->end_sem);
