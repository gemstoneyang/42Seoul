...
/*   Updated: 2022/05/23 16:15:34 by wonyang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <unistd.h>

void	decimal_to_hex(int decimal, int len)
{
	int		idx;
	int		mod;
	char	hexdecimal[16];
...
unsigned int	addr_size(char *addr)
{
	unsigned int	i;
...
void	*ft_print_memory(void *addr, unsigned int size)
{
	...
	{
		if (i % 16 == 0)
			decimal_to_hex((unsigned int)&address[i], 16);
		if (i < addr_size(address))
